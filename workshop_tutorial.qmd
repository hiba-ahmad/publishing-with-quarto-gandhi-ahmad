...
Download the quarto_tutorial.qmd file from the repository.

Once you have the .qmd file opened, complete the following exercises, so that the **code is reflected below in appropriate code chunks**:

1.  **Use at least 5 elements of markdown** (bold, italics, strike-through, adding table, images etc) in your .qmd file. You can use either the Source or Visual editor for this. What additional features do you see that we haven't discussed so far? List them down in the space below.
2.  **Render the document in your preferred way.** Verify that you can modify the code, re-run it, and see the modified output. You might have to authorize the installation of some packages.
3.  Render the document in a **format different than yours** (HTML, PFS, PPTX, revealjs).
4.  Create an account on Quarto Pub and **publish your file in at least 2 formats.**
5.  Add a **customizing option** of your preference **to the quarto publish command** in the command line. Re-run and verify that you can see the modified output.
6.  Make one of the published documents your **default website** and re-run to verify.
7.  Have you experienced/ can you think of any **limitations to Publishing with Quarto** so far? List them down in the space below.

{{< pagebreak >}}

## Solutions

### Additional features of a .qmd file

*(start writing here)*

### Limitations of Publishing with Quarto
